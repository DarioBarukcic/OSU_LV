...
plt.ylabel('x_2')
plt.legend(loc='upper left')
plt.title("Tocnost(KNN-train, n=7): " + "{:0.3f}".format((accuracy_score(y_train, y_train_p_KNN))))
plt.tight_layout()
plt.show()
...
KNN_gscv = GridSearchCV(estimator=KNN_model2, param_grid=param_grid,cv=5)
...
param_grid = {"C": np.arange(1,100), "gamma": [0.01,0.05,0.1,0.3,0.6,0.9,1]}
SVM_gscv = GridSearchCV(estimator=SVM_model2, param_grid=param_grid,cv=5)
